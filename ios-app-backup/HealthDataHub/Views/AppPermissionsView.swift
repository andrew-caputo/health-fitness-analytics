...
struct AppPermissionsView: View {
    ...
    private var enabledPermissionsCount: Int {
        permissionGroups.flatMap { $0.permissions }.filter { $0.status == .allowed }.count
    }
    
    private var deniedPermissionsCount: Int {
        permissionGroups.flatMap { $0.permissions }.filter { $0.status == .denied }.count
    }
...
struct PermissionGroup: Identifiable {
    ...
    var enabledCount: Int {
        permissions.filter { $0.status == .allowed }.count
    }
}
